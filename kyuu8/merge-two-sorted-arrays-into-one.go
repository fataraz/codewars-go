...
//one, sorted in asc order. Complete the function mergeArrays(arr1, arr2), where arr1 and arr2 are the original sorted arrays.
//
...
func MergeArrays(arr1, arr2 []int) (results []int) {
	ones := append(arr1, arr2...)
	sort.Ints(ones)
	for i, v := range ones {
		if i == 0 {
